...
# Source URLs for Sahih al-Bukhari (hadith-api)
ARA_URL = "https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/ara-bukhari.json"
ENG_URL = "https://cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/eng-bukhari.json"
OUTPUT_DIR = "/Users/saidamenmambayao/apps/transcriptions/shari'ah/hadith/bukhari"
...
def process_bukhari():
    ...
    for book_id, data in sorted(books_data.items(), key=lambda x: int(x[0])):
        ...
        nums = [h['num'] for h in data['hadiths']]
        h_start = min(nums)
        h_end = max(nums)
        ...
        content = [
            ...
            f"- **Source:** Sunnah.com Sahih al-Bukhari\n",
            f"\n",
            f"---\n\n"
        ]
        
        for h in sorted(data['hadiths'], key=lambda x: x['num']):
    ...
    print("Bukhari Processing Complete.")
