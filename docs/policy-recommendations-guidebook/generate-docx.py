#!/usr/bin/env python3
"""
Generate Policy Recommendations Guidebook DOCX from markdown chapter files.
...
OUTPUT_FILE = SCRIPT_DIR / "Policy-Recommendations-Guidebook-BARMM.docx"
...
def add_header_footer(doc):
    ...
    for section in doc.sections:
        ...
        run = p.add_run("Policy Recommendations Guidebook — Bangsamoro Government")
...
def create_title_page(doc):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("POLICY RECOMMENDATIONS")
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("A Practical Guide for Policy Analysts,\nParliamentary Staff, Ministry Officials, and Consultants")
