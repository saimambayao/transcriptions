#!/usr/bin/env python3
"""
Generate Speech Writing Guidebook DOCX from markdown chapter files.
...
OUTPUT_FILE = SCRIPT_DIR / "Speech-Writing-Guidebook-Bangsamoro-Parliament.docx"
...
def add_header_footer(doc):
    ...
    for section in doc.sections:
        ...
        run = p.add_run("Speech Writing Guidebook — Bangsamoro Parliament")
...
def create_title_page(doc):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("SPEECH WRITING GUIDEBOOK")
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("A Practical Guide for Legislative Staff,\nCommunications Officers, and Parliamentary Speechwriters")
