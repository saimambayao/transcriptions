...
**Matt's "grill me" session produced 8 clear requirements in 22 minutes -- this is the exact rigor your bill-drafting workflow needs.** When you draft legislation for BTA Parliament, the design tree approach (exhausting all decision branches before committing to text) maps perfectly to legislative drafting. A bill like the Cultural Heritage Act has cascading implications across BOL articles, existing BAAs, and BDP priorities. A structured grilling phase that asks "Does this provision conflict with Article X of the BOL?" before any section is written would catch issues that currently surface during committee review.

**The "ubiquitous language" concept from Domain-Driven Design is immediately applicable to your Bangsamoro platforms.** Matt maintains a glossary of terms (ghost lesson, materialize, materialization cascade) that both he and the AI share. Your platforms serve a domain with specialized vocabulary: BARMM governance terms, cooperative classifications, madrasah accreditation levels, OBC categories. Building a ubiquitous language file for each platform -- referenced in CLAUDE.md -- would eliminate the ambiguity that causes Claude Code to misinterpret domain-specific concepts like "SEED cooperative" or "tarbiyyah certification."

**The "day shift / night shift" pattern solves your fragmented schedule problem.** As OOBC Consultant by day and solo dev by night, you are already living this pattern informally. Matt's formalization -- human does the thinking (grilling, PRDs, issues) during the day, Ralph implements AFK during the night -- could be structured into your routine. Policy meetings and legislative research during work hours produce the requirements; Claude implements across your 9 platforms overnight. The 5-iteration, 6-commit result from 1.5 hours of AFK work demonstrates what is achievable.

**The QA loop revealing 7 issues in 8 minutes validates your /fact-checker approach, but suggests extending it to UI.** Your current workflow runs /fact-checker on documents, but Matt's QA process -- manually walking through the app, submitting bugs via a feedback button that creates GitHub issues -- is a pattern you could replicate with Playwright. Automated QA sweeps across your platforms (checking form submissions, role-based access controls, data integrity) would catch the edge cases that "specs-to-code will never fully predict."

**Matt's token efficiency observation (40k tokens after 22 minutes of grilling) reinforces your 1M context window advantage.** With Opus 4.6's 1M context, you can sustain much longer grilling sessions than most users. The explore sub-agent pattern (spawning agents that read files and return summarized context to the parent) keeps the main thread lean. This is directly applicable when working on complex bills that need to reference multiple BOL articles, existing BAAs, and BDP chapters simultaneously.

**The "focus on inputs and outputs, not code" principle applies to your legislative work too.** Matt reviews interfaces and module boundaries, not implementation details. When reviewing draft bills, you should focus on the interface between provisions (how Article 3 connects to Article 7, what the implementing rules require) rather than wordsmithing individual clauses. Let Claude handle the prose; you architect the policy structure.

### How This Can Improve Your Claude Skills and Workflows

**The /grill-me skill should be adapted for legislative design, not just software design.** Matt's 3-sentence skill forces exhaustive questioning before committing to code. Your legislative equivalent: "Interview me relentlessly about every aspect of this bill until we reach shared understanding of its policy implications. Walk down each branch of the legislative design tree, resolving dependencies between provisions one by one. If a question can be answered by consulting the BOL, BDP, or existing BAAs, consult those sources instead of asking me." This would catch constitutional conflicts and BDP misalignment before any bill text is written.

**The "ubiquitous language" pattern should be formalized across all your platforms.** Matt maintains a shared glossary for his app (ghost lesson, materialize, materialization cascade). Each of your 9 platforms needs the same: e-Bangsamoro (BAA, Resolution, plenary session, committee assignment, MANA assessment), Tarbiyyah-MS (asatidz, madrasah level, tarbiyyah certification), MoroMarket (cooperative tier, product catalog, SEED partner). Store each glossary in ~/Vault/projects/{platform}/ubiquitous-language.md and reference it in the platform's CLAUDE.md. This eliminates domain term ambiguity across Claude sessions.

**Matt's explore sub-agent pattern should be adopted by your /grill-me and /write-a-prd skills.** The explore phase (spawning a sub-agent to read many files and return summarized context to the parent agent) keeps the main thread lean at ~40k tokens. Your 1M context window gives you more room, but the discipline is still valuable: when /bill-drafter needs to reference 5 BOL articles, 3 existing BAAs, and 2 BDP chapters, an explore sub-agent that reads all sources and returns a focused summary preserves your main context for the actual drafting conversation.

**The "day shift / night shift" pattern should be codified as a workflow in your /devwork skill.** During OOBC consulting hours (day shift), you do the thinking: grilling, PRDs, policy analysis, requirements definition. These get committed as GitHub issues with acceptance criteria. During your evening coding sessions (night shift), Claude picks up unblocked issues and implements with TDD. Your /devwork skill could include a "queue mode" that lists all unblocked issues across your 9 platforms, prioritized by dependency chain, so you start each coding session knowing exactly what to build.

**The QA loop with a feedback button should be replicated with Playwright across your platforms.** Matt found 7 issues in 8 minutes through manual QA. Your 9 platforms need similar systematic QA, but you don't have time for manual walkthroughs of each. Build Playwright test suites that automate the QA sweep: login flows, RBAC verification, form submissions, data integrity checks. When tests fail, they automatically create GitHub issues that your "Ralph loop" equivalent picks up and fixes. This closes the loop between QA discovery and bug resolution without requiring your manual involvement.
