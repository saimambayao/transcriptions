...
**Matt Pocock's five-skill workflow is essentially what you have already built, but his naming conventions reveal refinement opportunities.** Your existing skill chain -- from /bangsamoro context loading through /bill-drafter, /fact-checker, and /humanizer -- follows the same discovery-to-implementation arc. The difference is Matt's explicit separation of "grill me" (shared understanding) from "write a PRD" (destination document). In your legislative workflow, the /bangsamoro skill loads context but doesn't force a structured interrogation phase before drafting begins. A dedicated /grill-me equivalent for bill drafting -- one that walks down the "design tree" of policy implications before any text is written -- could catch constitutional conflicts or BOL alignment issues earlier.

**The PRD-to-issues pattern maps directly to your 9-platform development workflow.** You are building e-Bangsamoro, e-Negosyo, Tarbiyyah-MS, and six other platforms as a solo dev. Matt's vertical slicing approach (thin slices through all integration layers, not horizontal layers) is critical when you are working across React 19 + Django + PostgreSQL. Each platform feature should be broken into tracer bullets that flush out integration unknowns between your frontend and Django REST APIs first, before building out the full UI.

**The TDD skill addresses your biggest solo-dev risk: regression across 9 codebases.** With no team to catch regressions, test-driven development is not optional -- it is your safety net. Matt's insight that "few large modules with thin interfaces" makes codebases more AI-navigable is especially relevant. Your platforms share common patterns (user auth, role-based access, data tables) that should be structured as deep, well-interfaced modules so Claude Code can work on any platform without re-learning the architecture every session.

**The /improve-codebase-architecture skill should run weekly across your active platforms.** Matt recommends running it after development surges. Given that you are shipping features across multiple codebases, scheduling a weekly architecture review -- spawning parallel sub-agents to propose interface improvements -- would prevent the technical debt that accumulates when a solo developer moves fast between projects. This directly supports your goal of building government-scale platforms that must be maintainable long-term.

**The "Ralph loop" concept (AFK agent implementation) is your force multiplier.** As someone who works across OOBC consulting, legislative research, MoroTech, and community leadership, your availability for hands-on coding is fragmented. Setting up an AFK loop that picks up GitHub issues, implements with TDD, and queues results for your review would let you do the thinking work (grilling, PRDs) during policy meetings and let Claude implement during your off-hours.

### How This Can Improve Your Claude Skills and Workflows

**Your /grill-me skill should be tested against Matt's 3-sentence original to determine if yours is over-engineered.** Matt's version is three sentences and produces 30-50 probing questions over 30-45 minutes. If your /grill-me skill has significantly more instructions, run a comparative /auto-research eval: same feature prompt, both skill versions, 10 runs each, measure (1) number of decision branches explored, (2) quality of final requirements, (3) token consumption. The simpler version may outperform -- less instruction means less constraint on Claude's reasoning.

**The /prd-to-issues skill's vertical slicing approach should be your standard for all platform development.** Matt's "tracer bullet" concept (thin slices through all integration layers, not horizontal layers) is critical for your React 19 + Django stack. When building a new e-Bangsamoro module (e.g., budget tracking), the first issue should be a thin vertical slice: one React component, one API endpoint, one Django model, one integration test. This flushes out unknown unknowns in the frontend-backend integration before you build out the full feature.

**The /tdd skill's "few large modules with thin interfaces" principle should inform your /improve-codebase-architecture runs.** Across your 9 platforms, you likely have the opposite: many small files with unclear boundaries, a pattern that makes AI navigation difficult. Schedule /improve-codebase-architecture weekly on your most active platform (probably e-Bangsamoro). The skill spawns 3+ sub-agents proposing radically different interface designs -- pick the best one and submit a refactor RFC. Over months, this compounds into codebases that are increasingly AI-navigable.

**Matt's skill sequence maps to a superpowered workflow chain for your platform development.** Your /sp-writing-plans superpower should encode this sequence: (1) /grill-me on the feature concept, (2) /write-a-prd to produce the destination document, (3) /prd-to-issues to create vertical slices with blocking relationships, (4) dispatch issues to parallel agents via /sp-dispatching-parallel-agents, (5) /tdd for each agent's implementation, (6) /sp-verification-before-completion for QA. This is the complete pipeline from idea to shipped feature, and each step produces artifacts that persist in GitHub for audit trails.

**The "garbage codebase = garbage AI output" insight means /improve-codebase-architecture is prerequisite, not optional.** Before running /auto-research on any platform-specific skill, ensure the codebase itself is well-structured. A skill optimized for a messy codebase will produce optimizations that only work in that mess. Clean the architecture first (deep modules, thin interfaces, ubiquitous language), then optimize the skills. The order matters.
