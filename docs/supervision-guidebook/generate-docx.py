#!/usr/bin/env python3
"""
Generate Supervision and Supervisory Development Guidebook DOCX from
...
OUTPUT_FILE = SCRIPT_DIR / "Supervision-Guidebook-BARMM.docx"

# Font settings — BARMM institutional palette
FONT_BODY = "Inter"
FONT_MONO = "Consolas"
...
def add_header_footer(doc):
    ...
    for section in doc.sections:
        ...
        run = p.add_run("Supervision and Supervisory Development Guidebook — BARMM")
...
def create_title_page(doc):
    ...
    set_run_font(run, FONT_BODY, size=11, bold=False, color=COLOR_GOLD)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("SUPERVISION AND")
    set_run_font(run, FONT_BODY, size=28, bold=True, color=COLOR_NAVY)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("SUPERVISORY DEVELOPMENT")
    set_run_font(run, FONT_BODY, size=28, bold=True, color=COLOR_NAVY)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("A Practical Guide for Directors, Division Chiefs,\nSection Heads, and Unit Supervisors")
    ...
    set_run_font(run, FONT_BODY, size=13, bold=False, color=COLOR_BODY)
...
def main():
    print("=" * 60)
    print("  Supervision Guidebook — DOCX Generator")
    print("=" * 60)
    print()
