...
**The /loop command is your deployment monitor for government-scale platforms.** When you deploy updates to e-Bangsamoro or any of your 9 platforms, `/loop 10m check deployment status and run Playwright tests on the login and dashboard routes` gives you automated monitoring during your work session. For platforms serving Bangsamoro government operations, catching downtime within 10 minutes rather than hours is the difference between a minor incident and a disruption to parliamentary workflows.

**Claude Code Desktop's /schedule command is better suited for your recurring legislative monitoring tasks.** Your role as OOBC Consultant requires tracking legislative developments, committee schedules, and bill status. A Desktop schedule that runs daily -- checking for new BAA filings, committee hearing notices, or BTA Parliament updates -- and saves summaries to your Obsidian vault would keep you informed without manual checking. The key advantage over /loop: no 3-day expiry, survives restart, and runs in a separate session so it does not interrupt active coding work.

**GitHub Actions is the right choice for CI/CD across your 9 platforms, not /loop.** Your React 19 + Django + PostgreSQL stack needs automated testing on every push. GitHub Actions running Claude Code for PR reviews, test execution, and deployment validation operates on GitHub's infrastructure -- your laptop can be closed. This is the persistent automation layer that /loop cannot provide. Reserve /loop for session-specific tasks like monitoring a migration script or babysitting a database seed operation.

**The practical use case hierarchy clarifies your automation strategy.** For your workflow: (1) GitHub Actions for CI/CD and PR automation across all repos, (2) Desktop /schedule for daily legislative monitoring and vault updates, (3) /loop for within-session tasks during active development sprints. Understanding these boundaries prevents the disappointment of trying to make /loop do what it was not designed for -- and saves you from re-creating scheduled tasks every 3 days.

### How This Can Improve Your Claude Skills and Workflows

**The /loop command should be integrated into your /auto-research and /skill-optimizer workflows.** Instead of manually re-invoking optimization runs, use `/loop 2m run next auto-research iteration on /bill-drafter` during a work session. The loop generates test outputs, evaluates them, mutates the prompt, and keeps the winner -- all while you focus on other work in a separate terminal. The 3-day max is more than enough for a single skill optimization sprint, and the loop's session-based nature means you can monitor progress in real time.

**Desktop /schedule is your tool for systematic vault maintenance.** Your /vault-update skill should be scheduled to run daily via Claude Code Desktop: check for stale session notes, verify INDEX.md is current, flag orphan files without wikilinks, and generate a daily vault health report saved to ~/Vault/daily/. This replaces ad-hoc vault maintenance with systematic care. The key advantage: it runs in a separate session, so it never interrupts your active coding or drafting work.

**The /loop + Playwright combination solves your cross-platform QA problem.** After deploying updates to any of your 9 platforms, `/loop 10m run Playwright tests on e-Bangsamoro login, dashboard, and legislative tracker` gives you automated regression monitoring for the duration of your work session. Create a Playwright test suite per platform (stored in each repo) that the loop can invoke. This is cheaper and more reliable than manual checking, and it catches regressions within 10 minutes rather than when a user reports them.

**GitHub Actions with Claude Code is the path to CI/CD across your platform portfolio.** Your 9 React + Django platforms share similar deployment patterns. A GitHub Actions workflow that runs Claude Code for PR review (checking for BOL-compliance in e-Bangsamoro, cooperative data integrity in MoroMarket, student privacy in Tarbiyyah-MS) would add domain-specific validation to your CI pipeline. This runs on GitHub's infrastructure -- no local machine needed -- and is the only persistent automation option in the current toolset.
