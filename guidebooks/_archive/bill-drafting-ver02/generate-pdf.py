#!/usr/bin/env python3
"""
Generate a professional PDF of the Bill Drafting Guidebook
for the Bangsamoro Parliament from Markdown source files.
...
OUTPUT_PDF = BASE_DIR / "Bill-Drafting-Guidebook-Bangsamoro-Parliament.pdf"
...
def build_toc_html(md_text: str) -> str:
    """Build a beautifully structured HTML Table of Contents from the
    markdown TOC file which uses &emsp; for indentation levels.

    The source format is:
      **Chapter Title**
      &emsp;1.1 Section
      &emsp;&emsp;1.1.1 Subsection
      &emsp;&emsp;&emsp;1.1.1.1 Sub-subsection
    """
    lines = md_text.split("\n")
    html_parts = []

    # Extract the main title and subtitle (H1 and H2) first
    title_done = False
    subtitle_done = False
    toc_started = False
    ...
    for line in lines:
        ...
        # H1 title - skip it (the cover page handles this)
        ...
        # Count &emsp; to determine indentation level
...
def wrap_section(fname: str, html_content: str, index: int) -> str:
    ...
    # Introduction (first real chapter)
...
def post_process_html(html: str) -> str:
    ...
    # Handle HTML-escaped versions (markdown converters may escape comments)
    html = html.replace("&lt;!-- LANDSCAPE START --&gt;", '<div class="landscape-page">')
    html = html.replace("&lt;!-- LANDSCAPE END --&gt;", '</div>')

    # Convert checkbox patterns [ ] to Unicode ballot box
    ...
    # Tag checklist tables (5-col tables with Yes/No/N/A headers) for narrow column styling
    html = re.sub(
...
def build_full_html(sections: list[tuple[str, str]]) -> str:
    ...
    # Convert and wrap each section
    all_sections_html = []
    for i, (fname, md_content) in enumerate(sections):
        if "table-of-contents" in fname:
            # Use custom TOC builder for proper hierarchical formatting
...
def generate_pdf_playwright(html_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Try alternative playwright approach
...
def main():
    print("=" * 60)
    print("  Bill Drafting Guidebook — PDF Generator")
    print("=" * 60)
    print()
    ...
    # Step 3: Save intermediate HTML (for debugging and fallback)
