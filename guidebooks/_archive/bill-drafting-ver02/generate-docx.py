#!/usr/bin/env python3
"""
Generate Bill Drafting Guidebook DOCX from markdown chapter files.
...
OUTPUT_FILE = SCRIPT_DIR / "Bill-Drafting-Guidebook-Bangsamoro-Parliament.docx"
...
def add_header_footer(doc):
    ...
    for section in doc.sections:
        ...
        run = p.add_run("Bill Drafting Guidebook — Bangsamoro Parliament")
...
def create_title_page(doc):
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("BILL DRAFTING GUIDEBOOK")
    set_run_font(run, FONT_BODY, size=28, bold=True, color=COLOR_NAVY)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("FOR THE BANGSAMORO PARLIAMENT")
    set_run_font(run, FONT_BODY, size=28, bold=True, color=COLOR_NAVY)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("A Practical Guide for Members of Parliament,\nParliamentary Staff, and Bill Drafters")
