#!/usr/bin/env python3
"""
Generate CSW Guidebook DOCX from markdown chapter files.
...
OUTPUT_FILE = SCRIPT_DIR / "CSW-Guidebook-Bangsamoro-Autonomous-Region.docx"
...
def add_header_footer(doc):
    ...
    for section in doc.sections:
        ...
        run = p.add_run("CSW Guidebook — Bangsamoro Autonomous Region")
...
def create_title_page(doc):
    ...
    set_run_font(run, FONT_BODY, size=11, color=COLOR_GOLD)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("COMPLETE STAFF WORK")
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("A Practical Guide for Parliament Staff,\nMinistry Personnel, and Government Officers")
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("The ADDRESS IT Framework")
    set_run_font(run, FONT_BODY, size=11, color=COLOR_GOLD)
