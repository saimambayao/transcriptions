#!/usr/bin/env python3
"""
Generate a professional PDF of the Supervision and Supervisory Development
Guidebook for the Bangsamoro Autonomous Region from Markdown source files.
...
OUTPUT_PDF = BASE_DIR / "Supervision-Guidebook-BARMM.pdf"
...
def build_chapter_cover(fname: str, md_content: str) -> str:
    ...
    # No cover page for appendix files — they flow continuously
    ...
    is_back_matter = any(k in fname for k in ["glossary"])
...
def build_toc_html(md_text: str) -> str:
    ...
    for line in lines:
        ...
        if is_chapter:
            ...
            html_parts.append(
                f'<div class="toc-chapter"><a href="#{anchor}">{entry_text}</a></div>'
            )
        else:
            if emsp_count >= 3:
...
def main():
    print("=" * 60)
    print("  Supervision Guidebook — PDF Generator")
    print("=" * 60)
    print()
