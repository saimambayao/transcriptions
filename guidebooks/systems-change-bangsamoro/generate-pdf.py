#!/usr/bin/env python3
"""
Generate a professional PDF of the Systems Change in the Bangsamoro guidebook
...
OUTPUT_PDF = BASE_DIR / "Systems-Change-in-the-Bangsamoro.pdf"
...
def check_mermaid_blocks(sections: list[tuple[str, str]]):
    """Warn about any remaining ```mermaid code blocks that should be pre-rendered as PNGs."""
    for fname, content in sections:
        mermaid_matches = re.findall(r'```mermaid', content)
        if mermaid_matches:
            count = len(mermaid_matches)
            print(f"  [WARN] {fname} contains {count} ```mermaid block(s).")
            print(f"         These should be pre-rendered as PNG images before PDF generation.")
            print(f"         Mermaid code blocks will appear as raw text in the PDF.")
...
def main():
    print("=" * 60)
    print("  Systems Change in the Bangsamoro — PDF Generator")
    print("=" * 60)
    print()

    # Step 1: Read markdown files
    print("[1/5] Reading markdown files...")
    ...
    # Step 2: Check for Mermaid blocks
    print("[2/5] Checking for unrendered Mermaid diagrams...")
    check_mermaid_blocks(sections)
    print()
    ...
    # Step 4: Save intermediate HTML (for debugging and fallback)
    print("[4/5] Saving intermediate HTML...")
