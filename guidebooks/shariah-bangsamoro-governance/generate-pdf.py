#!/usr/bin/env python3
"""
Generate a professional PDF of Shari'ah and Bangsamoro Governance
...
OUTPUT_PDF = BASE_DIR / "Shariah-and-Bangsamoro-Governance.pdf"
OUTPUT_HTML = BASE_DIR / "guidebook-rendered.html"  # intermediate, for debugging

# Ordered list of markdown files (ver03 includes appendices)
MD_FILES = [
...
def build_chapter_cover(fname: str, md_content: str) -> str:
    ...
    is_appendix = "appendix" in fname

    # No cover pages for back matter or appendices — they flow continuously
    if is_appendix or is_back_matter:
        return ""

    if not is_chapter and not is_front_matter:
        return ""
...
def build_toc_html(md_text: str) -> str:
    ...
    for line in lines:
        ...
        # Handle ### Part headers (e.g., "### Part I — Foundations")
        if stripped.startswith("### "):
            part_text = stripped.lstrip("# ").strip()
            html_parts.append(f'<div class="toc-part">{part_text}</div>')
            continue
...
def wrap_section(fname: str, html_content: str, index: int) -> str:
    ...
    # About / Author pages — frontmatter style
    if fname in ("00b-about.md", "00c-author.md"):
        page_id = fname.replace(".md", "").replace("-", "-")
        return f'<div class="frontmatter-page" id="{page_id}">\n{html_content}\n</div>\n'
    ...
    # Appendices — flow continuously, no separate page break
...
def main():
    print("=" * 60)
    print("  Shari'ah and Bangsamoro Governance — PDF Generator")
    print("=" * 60)
    print()
