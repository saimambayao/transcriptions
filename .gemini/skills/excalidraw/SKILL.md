...
name: excalidraw
description: |
  Excalidraw diagram generator for architectural diagrams, mind maps, swimlanes,
  process flows, and any visual that needs more control than Mermaid provides.
  Use when: Mermaid limitations are hit (fan-out patterns, complex styling,
  multi-color categorization, mind maps, swimlanes), or user explicitly requests
  Excalidraw, "hand-drawn diagram", "better diagram", "diagram with colors",
  "visual architecture", "excalidraw".
  Complements Mermaid — use Mermaid for simple linear flowcharts and sequence diagrams.
...
**Renderer:** `~/.gemini/skills/excalidraw/scripts/venv/bin/python3 ~/.gemini/skills/excalidraw/scripts/render_excalidraw.py input.excalidraw -o output.png`

**Color palette:** Always read `references/color-palette.md` before generating. Default is BARMM institutional palette (Navy #1B365D, Gold #C5A54E, Slate #2C5F7C). Use `roughness: 0` for government documents.

**Default diagramming tool.** Use Excalidraw for ALL diagrams — simple and complex. It produces consistently better visual output than Mermaid. Only fall back to Mermaid for sequence diagrams or ER diagrams where Mermaid has native syntax advantages.
...
**For comprehensive or technical diagrams, you MUST build the JSON one section at a time.** Do NOT attempt to generate the entire file in a single pass. This is a hard constraint — Gemini CLI has a ~32,000 token output limit per response, and a comprehensive diagram easily exceeds that in one shot. Even if it didn't, generating everything at once leads to worse quality. Section-by-section is better in every way.
...
```bash
cd .gemini/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
```bash
cd .gemini/skills/excalidraw-diagram/references
uv sync
