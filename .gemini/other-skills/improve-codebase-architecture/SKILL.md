...
description: |
  ...
  architecture, find refactoring opportunities, consolidate tightly-coupled modules, make a
  codebase more AI-navigable, or mentions "improve architecture", "deepen modules",
  "codebase health", "architectural review", or "find refactoring opportunities".
  Run this weekly or after a surge of development to maintain codebase quality.
...
- **Previous step**: `/tdd` — this skill improves the codebase that TDD implementations build on
- **Next step**: `/grill-me` — use the refactor RFC to start a new feature cycle with a grilling session
- **Full cycle**: `/grill-me` → `/write-a-prd` → `/prd-to-issues` → `/tdd` → `/improve-codebase-architecture` → repeat
